...
This challenge gives us a USB packet-captured file.
Brief surveys allowed us to reach [this post](https://blog.stayontarget.org/2019/03/decoding-mixed-case-usb-keystrokes-from.html) but it could not immediately retrieved the flag due to the blank lines (it was included in the text file obtained via `tshark -r chall.pcap -T fields -e usb.capdata | tr -d : > usb_capdata.txt`).
Thus, we added a few modifications to the code and the succeeded one is as follows.
